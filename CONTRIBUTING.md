...
Prism is a family dashboard, so almost every fixture, screenshot and doc example
is shaped like a real household. Use the standing cast rather than inventing new
names, and never use a real person, address, school or network:
...
| Surname | Parker |
| Emails | `name@example.com` (RFC 2606, reserved for documentation) |
| Phone numbers | `(555) 01xx` (reserved for fiction) |
| Address / city | Springfield, IL |
| Coordinates | A national park or city centre, never a residential precision |
| Hostnames | `prism.local`, `homeassistant.local`, `your-domain.com` |
| Private IPs | `192.168.x.x` with the octets masked, outside `src/` |

Two reasons this is a rule and not a preference. A name that reads as a person
is indistinguishable from a real one to any scanner, so the cast is effectively
an allowlist: an unfamiliar name stands out during review. And screenshots are
published to the docs site, where no scanner can read a pixel at all, so the
only control is that the data behind them was never real.

The same goes for anything you write around the code. A pull request body, an
issue comment and a commit message are as public as the diff, and a commit
message cannot be edited once it is pushed. Do not paste real addresses, real
network addresses, log excerpts from a live instance, or another person's email
into any of them, including your own. If a real value is needed to explain
something, describe it rather than quoting it.

Screenshots in `docs/demos/` are generated by CI from a seeded database
(`.github/workflows/refresh-screenshots.yml`). Do not commit screenshots taken
by hand; `scripts/capture-screenshots.ts` refuses to run without
`E2E_HAS_TEST_DB=1` for that reason.
