...
is indistinguishable from a real one to every scanner in this repo, so the cast
is effectively an allowlist: an unfamiliar name stands out during review. And
screenshots are published to the docs site, where no scanner can read a pixel at
all, so the only control is that the data behind them was never real.
