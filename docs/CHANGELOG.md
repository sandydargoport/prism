...
### Added — Settings
- **Consolidated Integrations page** (`/settings?section=integrations`): one card per provider brand — Google (Calendars + Tasks + Bus tracking via Gmail), Microsoft (incl. OneDrive), Apple/CalDAV, Kroger — plus a cross-provider Photo Sources card. Each card has a connection status badge and collapsible sub-sections for per-feature wiring, with per-sub-section status badges so users wiring different accounts for different services still see the truth. URL anchors (`#microsoft-onedrive`, `#google-bus`, `#caldav-calendars`, etc.) deep-link straight to a sub-section. Ships alongside the legacy Connected Accounts / Task Sync / Shopping Sync / Wish List Sync / Photos sections; cleanup pass removing the legacy sections will follow once parity is verified on real accounts. Closes phase 1 of [#52](https://github.com/sandydargoport/prism/issues/52).
